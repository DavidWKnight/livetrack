...
transmitterEl = [40, 50] # Testing values for now
...
try:
    while True:
        ...
        for icao in trackedAircraft:
            if (datetime.now() - trackedAircraft[icao]).total_seconds() > aicraftTimeout:
        ...
        if aircraftAER[1] > transmitterEl[0] and aircraftAER[1] < transmitterEl[1]:
            if icao not in trackedAircraft:
                fname = now.isoformat(timespec='seconds') + "_" + icao + '.dat'
                pluto.startRecording(icao, fname)
                print(f"{icao} entered")
            
            trackedAircraft[icao] = now
